program main2
    ...
    real :: Winh(N) = 2.5        ! peso inputs inhibidores para cada neurona
    real :: z(N) = 0.           ! parametro filtro disparos
    real, parameter :: tauw = 4000.   ! constante de tiempo decaimiento pesos
    real, parameter :: taul = 20.   ! constante tiempo filtro disparos
    real, parameter :: etaE = 0.0005
    real, parameter :: etaI = 0.0005
    ...
    do l = 1, numgamma
        ...
        do idx = 1, intentos
                ...
                do j = 1, N
                    ...
                    do k = 1, nnnumber(j)
                        ...
                        if (etiq(nn(j,k))==1) then  ! si el vecino k-esimo es excitador
                            ...
                            cI = float(etiq(nn(j,k)) * statuspre(nn(j,k))) * Winh(j)
                            lambda = lambda + cI
                            if (i>20000) then
                    ...
                    if (i>burn) then
                        if (etiq(j)==1) then
                            Winh(j) = Winh(j) - Winh(j) / tauw + etaE * z(j)
                        else
                            Winh(j) = Winh(j) - Winh(j) / tauw + etaI * z(j)
                        endif
                        z(j) = z(j) - z(j) / taul
                    endif
                enddo
            ...
            do i = 1, pasos
                ro(i) = ro(i) / float(N)
                Wimean(i) = Wimean(i) / float(N)
                ! write(0,*) i, ro(i), Wimean(i)
                if (i>20000) then
                    amean = amean + ro(i)
                endif
            enddo
            ...
            do i = 1, N
                ffreq(i) = ffreq(i) / float(20000)
                inputE(i) = inputE(i) / float(20000)
                inputI(i) = inputI(i) / float(20000)
