program main2
    ...
    integer, parameter :: pasos = 190000 ! 1000000
    integer, parameter :: burn = 15000   ! pasos desechados para calcular observables
    integer, parameter :: intentos = 30 !10 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    write(name, '("r",I3,".txt")') argint

    open(1,status='replace', file=name)
    do l = 1, numgamma
        ! gamma = 0.14 + ((0.32 - 0.14) / float(numgamma-1))*float(l - 1)
        ! gamma = 1.9 + float(argint-1)*(3.4 - 1.9)/70.
        gamma = float(argint)/100.0         ! 1.68 active? 1.66 inactive?
        write(*,*) gamma
        ! write(*,*) 'gamm', gamma, 'sigE', sigmae, 'sigI', sigmai, 'we', we, 'wi', wi, 'pt', pt, 'epsil', epsilon
        ! write(1,*) '# gamm', gamma, 'sigE', sigmae, 'sigI', sigmai, 'we', we, 'wi', wi, 'pt', pt, 'epsil', epsilon
        mamean = 0.
        ...
        do idx = 1, intentos
            ...
            do j = 1, N
                write(1,*) float(ffreq(j))/float(pasos-burn), input(j)/float(pasos-burn)
            enddo
