    ...
    real:: pt = 1.0     ! probabilidad de teletransportar neurona == epsilon
    real, parameter :: epsilon = 0.0
    character (len=15)::nombrefoto
    ...
    call get_command_argument(1,argumento)  !gamma 
    call get_command_argument(2,argumento2) !pt == epsilon
    read(argumento,*) argint         
    read(argumento2,*) argint2

    
    write(nombrefoto, '("fa",I2,"-",I2,".txt")')argint,argint2
    gamma = 1.16 + (1.7-1.16) * float(argint) / 70.0
    pt = float(argint2) / 40.0
    if ((gamma>1.15).and.(gamma<1.54)) then
        intentos = 180
    endif

    open(1,status='replace', file=nombrefoto)
